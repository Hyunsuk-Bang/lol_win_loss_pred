---
title: "League of Legends Win/Loss Predictor"
author: "Hyunsuk Bang"
output:
  html_document:
    toc: yes
  ...
  pdf_document:
    toc: yes
---
##League of Legends Win/Loss Predictors

###Read csv and prepare for training
```{r}
...
###prepare function that creates randomForest model
```{r}
...
###print best model with max balanced accuracy and minimum OOB
```{r} 
...
###Results with ntree = 1000 & mtry = 6
```{r}
model <- learn_model(train, 1000, 6)
importance(model)
varImpPlot(model)
pred <- predict(model, newdata = test, type= "class")
